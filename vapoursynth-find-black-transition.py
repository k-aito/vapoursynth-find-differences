...
# -*- coding: utf-8 -*-
#
#  vapoursynth-find-black-transition.py
#  
...
"""
Vapoursynth-find-black-transition.py can be used to find where there are black frames in a video
- Compare each frames to a black frames
...
parser.add_argument("-p", "--percent",
                    required=False,
                    help="Percent of differnce between video and black frames, 0 minimum and 1 maximum (default 0.1)")

# OPTIONAL: -v / --verbose
parser.add_argument("-v", "--verbose",
                    required=False,
                    help="Enable the progression while scanning each frames (default: disabled)",
                    action='store_true')
...
print("INFO: Start the comparaison of each frames to a black frame")
for i in range(0, diff.num_frames):
  if args.verbose:
    print('{} / {}'.format(i, diff.num_frames))
  else:
    if i % 1000 == 0:
...
for i,value in enumerate(sceneFrames):
  ...
  if nextValueGuess == nextValue:
    ...
    # Method to conver seconds in hh:mm:ss
...
print("INFO: Intervals that are maybe black transitions")
import pprint
